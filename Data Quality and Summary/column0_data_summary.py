...
if __name__ == "__main__":
 ...
 cmplnt = cmplnt.map(lambda x: (check(x[0],x[1]),1))
 cmplnt = cmplnt.reduceByKey(lambda x,y: x+y)
 cmplnt =cmplnt.map(lambda x: '%s\t%s' %(x[0],x[1]))
 

 cmplnt.saveAsTextFile("column0_data_summary.out") 
